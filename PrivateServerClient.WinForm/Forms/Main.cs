    ...
    public partial class Main : Form
    {
        ...
        private void GenerateUserBestPerformance()
        {
            ...
            for (int i = 0; i < _playerBestPerformanceData.Scores.Count; i++)
            {
                ...
                bestPerformanceList.Items.Add($"#{i + 1}  |  ★{score.Beatmap.Difficulty:F2} - {score.PerformancePoints:F}pp - {score.Accuracy:F2}% - {TextUtils.CheckString(score.Beatmap.Title)} by {TextUtils.CheckString(score.Beatmap.Artist)} ({TextUtils.CheckString(score.Beatmap.Version)})");
            }
        }
